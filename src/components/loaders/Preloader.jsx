...
function PreloaderWindow({ title, subtitle, logoOffset, setDidLoadAllImages, showElements, isHiding }) {
    ...
    const [isAnimationHidden, setIsAnimationHidden] = useState(true)
    ...
    useEffect(() => {
        ...
        scheduler.clearAllWithTag("preloader-animation")
        scheduler.schedule(() => {
            setIsAnimationHidden(false)
        }, 100, "preloader-animation")
    }, [showElements])

    return (
        <div className={`preloader-window ${hiddenClass}`}>
            <div className={`preloader-window-content`}>
                {/* --- Naya Cool Spinner/Pulse Animation --- */}
                <div className={`cool-loader-container ${isAnimationHidden ? 'd-none' : ''}`}>
                    <div className="cool-pulsing-ring"></div>
                </div>
